...
\begin{document}
	...
	\begin{enumerate}
		...
		\item \textbf{Error State Estimation:} Predict the growth of error uncertainty, compute the Kalman gain using incoming measurements, and estimate the optimal error state $\delta\hat{\mathbf{x}}_{k+1}$.
	...
	\begin{itemize}
		\item The uncertainty of the error state propagation, represented by the prior error covariance matrix $\mathbf{P}_{k+1}^-$.
		\item The noise characteristics of the physical sensor, represented by the measurement noise covariance matrix $\mathbf{R}_{k+1}$.
	\end{itemize}
	...
	We know that:
	\begin{equation*}
		\frac{\partial f(\mathbf{x})}{\partial \delta\mathbf{x}} = \frac{\partial f(\mathbf{x})}{\partial \mathbf{x}} \cdot \frac{\partial \mathbf{x}}{\partial \delta\mathbf{x}} = \frac{\partial f(\mathbf{x})}{\partial \mathbf{x}} \cdot \frac{\partial (\hat{\mathbf{x}} + \delta\mathbf{x})}{\partial \delta\mathbf{x}} = \frac{\partial f(\mathbf{x})}{\partial \mathbf{x}} \mathbf{I}
	\end{equation*}
	
	This means that:
	\begin{equation*}
		\mathbf{F}_k = \left. \frac{\partial f}{\partial \mathbf{x}} \right|_{\hat{\mathbf{x}}_k^+, \mathbf{u}_k} =  \left. \frac{\partial f}{\partial \mathbf{\delta x}} \right|_{\mathbf{\delta x} = 0, \mathbf{u}_k}
	\end{equation*}
	We will generally use the last form (w.r.t $\delta x$) too derive $\mathbf{F}_k$
	...
	The Kalman gain matrix $\mathbf{K}$ is computed as:
