...
\begin{document}
	...
	To handle non-linear manifold topologies—such as 3D rotations—and improve numerical stability, the Error-State Kalman Filter (ESKF) decouples the true state $\mathbf{x}$ into a large-signal **nominal state** $\hat{\mathbf{x}}$ and a small-signal **error state** $\delta\mathbf{x}$:
	
	\begin{equation}
		\mathbf{x} = \hat{\mathbf{x}} \oplus \delta\mathbf{x}
	\end{equation}
	
	where $\oplus$ denotes a generalized composition operator (e.g., standard addition for vector spaces or quaternion multiplication for rotations).
	...
	\subsubsection{Measurement Residual}
	
	The residual is computed as:
	\begin{equation*}
		y = \mathbf{z}_{k+1} - \mathbf{\hat{z}}_{k+1}
	\end{equation*}
	Where $\mathbf{z}_{k+1}$ is the physical measurement we make. The mapping to the state space is described using the non-linear measurement equation $h$:
	\begin{equation*}
		\mathbf{z}_{k+1} = h(x_{k+1}) + v_{k+1}
	\end{equation*}
	$v_{k+1}$ is some zero mean noise which is added as the measurement of the state is not perfect.
	
	
	We can think of $\mathbf{\hat{z}}_{k+1}$ as the measurement we would make given the state $\hat{x}_{k+1}$. We can also compute $\mathbf{\hat{z}}_{k+1}$ using the non-linear 
	measurement equation $h$:
	\begin{equation*}
 		\mathbf{\hat{z}}_{k+1} = h(\hat{x}_{k+1})
	\end{equation*}
	
	Substitution leads us to the equation we can use to compute the value of the residual:
	\begin{equation*}
		y = \mathbf{\hat{z}}_{k+1} - h(\hat{x}_{k+1})
	\end{equation*}
	
	Now that we know the value of the residual an important metric we need too know is the uncertainty of the residual. Recall the Kalman gain is a weighting matrix identyfing if we need too trust the model or the measurements more. Its main inputs are the model uncertainty (identified by P). Too obtain this uncertainty we will approximate the non-linear residual function (PUT reference here) with a first order Taylor series expansion.	
	
	When a new measurement $\mathbf{z}_{k+1}$ arrives, the measurement function $h(\mathbf{x}_{k+1})$ is linearized around the propagated nominal state $\hat{\mathbf{x}}_{k+1}$ using a first-order Taylor series approximation:
	
	\begin{align}
		\mathbf{z}_{k+1} &= h(\mathbf{x}_{k+1}) + \mathbf{v}_{k+1} \nonumber \\
		&= h(\hat{\mathbf{x}}_{k+1} + \delta\mathbf{x}_{k+1}) + \mathbf{v}_{k+1} \nonumber \\
		...
		&\approx h(\hat{\mathbf{x}}_{k+1}) + \mathbf{H}_{k+1} \delta\mathbf{x}_{k+1} + \mathbf{v}_{k+1}
	\end{align}
	
	Defining the measurement Jacobian matrix as $\mathbf{H}_{k+1} = \left. \frac{\partial h}{\partial \mathbf{x}} \right|_{\hat{\mathbf{x}}_{k+1}}$, the measurement residual (or innovation) $\mathbf{y}_{k+1}$ represents the discrepancy between the physical observation and the predicted nominal measurement:
	
	\begin{equation}
		\mathbf{y}_{k+1} = \mathbf{z}_{k+1} - h(\hat{\mathbf{x}}_{k+1}) \approx \mathbf{H}_{k+1} \delta\mathbf{x}_{k+1} + \mathbf{v}_{k+1}
	\end{equation}
	
	Please note we can compute the actual residual by doing 
	...
	Finally, the optimal Kalman gain $\mathbf{K}_{k+1}$ is computed to weigh the innovation when estimating the error state:
