...
\begin{document}
	...
	In this section, we derive the state propagation function $f$. Recall that this function updates the full nominal state vector $\mathbf{x}_k = [\mathbf{p}_k^T, \mathbf{v}_k^T, \mathbf{q}_k^T, \mathbf{b}_{a,k}^T, \mathbf{b}_{g,k}^T]^T$ over a discrete time step $\Delta t$:
	\begin{equation}
		\mathbf{x}_{k+1} = f(\mathbf{x}_k, \mathbf{u}_k) + \mathbf{w}_k
	\end{equation}
	
	The vector-valued function $f$ is partitioned into component-wise sub-functions according to each state variable:
	\begin{equation}
		f(\mathbf{x}_k, \mathbf{u}_k) = 
		\begin{bmatrix}
			f_p(\mathbf{x}_k, \mathbf{u}_k) \\
			f_v(\mathbf{x}_k, \mathbf{u}_k) \\
			f_q(\mathbf{x}_k, \mathbf{u}_k) \\
			f_{b_a}(\mathbf{x}_k, \mathbf{u}_k) \\
			f_{b_g}(\mathbf{x}_k, \mathbf{u}_k)
		\end{bmatrix} =
		\begin{bmatrix}
			\mathbf{p}_k + \mathbf{v}_k \Delta t \\
			\mathbf{v}_k + \left( \mathbf{R}(\hat{\mathbf{q}}_k)\mathbf{a}_b + \mathbf{g} \right) \Delta t \\
			\mathbf{q}_k \otimes \mathbf{q}\left(\boldsymbol{\omega}_b \Delta t\right) \\
			\mathbf{b}_{a, k} \\
			\mathbf{b}_{g, k}
		\end{bmatrix}
	\end{equation}
	
	Each sub-function is detailed in the following subsections.
	...
	The measurement function $h: \mathcal{X} \to \mathcal{Z}$ maps the nominal state vector $\mathbf{x} = [\mathbf{p}^T, \mathbf{v}^T, \mathbf{q}^T, \mathbf{b}_a^T, \mathbf{b}_g^T]^T$ into the expected measurement space $\mathcal{Z}$:
	\begin{equation}
		\mathbf{z} = h(\mathbf{x}) + \mathbf{v}
	\end{equation}
	where $\mathbf{v}$ represents zero-mean measurement noise. Depending on the active sensor suite, $h(\mathbf{x})$ is defined as a composite system of sub-functions corresponding to each sensor:
	\begin{equation}
		h(\mathbf{x}) = 
		\begin{bmatrix}
			h_{\text{GPS}}(\mathbf{x}) \\
			h_{\text{baro}}(\mathbf{x}) \\
			h_{\text{mag}}(\mathbf{x}) \\
			h_{\text{acc}}(\mathbf{x})
		\end{bmatrix} =
		\begin{bmatrix}
			\mathbf{p} \\
			p_z \\
			\mathbf{R}(\mathbf{q})^T \mathbf{m}_n \\
			-\mathbf{R}(\mathbf{q})^T \mathbf{g}_n
		\end{bmatrix}
	\end{equation}
	
	The individual measurement models are defined as follows:
	
	\begin{itemize}
		\item \textbf{GPS ($\mathbf{z}_{\text{GPS}}$):} Directly measures the 3D position vector in the navigation frame $n$:
		\begin{equation}
			h_{\text{GPS}}(\mathbf{x}) = \mathbf{p} = [p_x, p_y, p_z]^T
		\end{equation}
		
		\item \textbf{Barometric Altimeter ($z_{\text{baro}}$):} Measures the 1D vertical height along the $z$-axis of the navigation frame:
		\begin{equation}
			h_{\text{baro}}(\mathbf{x}) = p_z
		\end{equation}
		
		\item \textbf{Magnetometer ($\mathbf{z}_{\text{mag}}$):} Measures Earth's magnetic field vector $\mathbf{m}_n$ mapped from the navigation frame to the body-fixed frame $b$ using the conjugate rotation $\mathbf{R}(\mathbf{q})^T$:
		\begin{equation}
			h_{\text{mag}}(\mathbf{x}) = \mathbf{R}(\mathbf{q})^T \mathbf{m}_n
		\end{equation}
		
		\item \textbf{Accelerometer ($\mathbf{z}_{\text{acc}}$):} Under quasi-static conditions (negligible body acceleration), the accelerometer measures the reaction to Earth's gravity vector $\mathbf{g}_n$ transformed into the body frame:
		\begin{equation}
			h_{\text{acc}}(\mathbf{x}) = -\mathbf{R}(\mathbf{q})^T \mathbf{g}_n
		\end{equation}
	\end{itemize}
	
	\subsection{Computation of the residual}
	
	The innovation (or residual) vector represents the discrepancy between the actual sensor measurements $\mathbf{z}_{k+1}$ and the predicted measurements $h(\hat{\mathbf{x}}_{k+1}^-)$. For vector space components (such as position and velocity), the residual is computed via standard Euclidean subtraction:
	\begin{equation}
		\mathbf{y}_{k+1} = \mathbf{z}_{k+1} - h(\hat{\mathbf{x}}_{k+1}^-)
	\end{equation}
	
	However, when the measurement space contains rotational quantities—such as orientation quaternions or vector observations transformed by orientation—standard linear subtraction is mathematically invalid. Unit quaternions belong to the non-Euclidean Lie group $SO(3)$, where linear subtraction does not preserve the unit-norm constraint and fails to capture the true angular distance.
	
	To properly compute the difference between two rotations, we must use quaternion composition. The rotational residual is defined via the relative rotation (or error rotation) operator, often denoted using the manifold "box-minus" ($\ominus$) notation:
	\begin{equation}
		\delta \mathbf{q} = \mathbf{z}_{\text{rot}} \ominus h_{\text{rot}}(\hat{\mathbf{x}}_{k+1}^-) = \mathbf{z}_{\text{rot}} \otimes h_{\text{rot}}(\hat{\mathbf{x}}_{k+1}^-)^{-1}
	\end{equation}
	where $\otimes$ denotes quaternion multiplication and $(\cdot)^{-1}$ denotes the quaternion conjugate.
	
	For injection into the linear Kalman filter framework, this quaternion error $\delta \mathbf{q} = [q_w, \delta \boldsymbol{\theta}^T/2]^T$ is typically mapped to a 3D minimal vector space representation $\delta \boldsymbol{\theta} \in \mathbb{R}^3$ (using the vector part or axis-angle mapping):
	\begin{equation}
		\mathbf{y}_{\text{rot}} = 2 \cdot \text{vec}(\delta \mathbf{q}) \approx \delta \boldsymbol{\theta}
	\end{equation}
	
	Thus, the overall residual vector $\mathbf{y}_{k+1}$ is constructed block-wise, employing Euclidean subtraction for vector quantities and rotational manifold differences for angular quantities.
	...
	\subsection{The process noise covariance matrix $\mathbf{Q}$}

	
	Consider the continuous error-state dynamics driven by physical IMU noise $\mathbf{w}(t) \in \mathbb{R}^{12}$:
	\begin{equation}
		\delta\dot{\mathbf{x}}(t) = \mathbf{F}_c \, \delta\mathbf{x}(t) + \mathbf{L}_c \, \mathbf{w}(t)
	\end{equation}
	
	\subsubsection{Role and Definition of $\mathbf{L}_c$ and Continuous Noise $\mathbf{Q}_c$}
	
	The raw continuous physical noise vector $\mathbf{w}(t) = [\mathbf{n}_a^T, \, \mathbf{n}_g^T, \, \mathbf{w}_{ba}^T, \, \mathbf{w}_{bg}^T]^T \in \mathbb{R}^{12}$ represents the white noise processes driving the sensors, modeled by the continuous Power Spectral Density (PSD) covariance matrix $\mathbf{Q}_c \in \mathbb{R}^{12 \times 12}$:
	
	\begin{equation}
		\mathbf{Q}_c = \begin{bmatrix}
			\sigma_a^2 \mathbf{I}_{3 \times 3} & \mathbf{0}_{3 \times 3} & \mathbf{0}_{3 \times 3} & \mathbf{0}_{3 \times 3} \\
			\mathbf{0}_{3 \times 3} & \sigma_g^2 \mathbf{I}_{3 \times 3} & \mathbf{0}_{3 \times 3} & \mathbf{0}_{3 \times 3} \\
			\mathbf{0}_{3 \times 3} & \mathbf{0}_{3 \times 3} & \sigma_{ba}^2 \mathbf{I}_{3 \times 3} & \mathbf{0}_{3 \times 3} \\
			\mathbf{0}_{3 \times 3} & \mathbf{0}_{3 \times 3} & \mathbf{0}_{3 \times 3} & \sigma_{bg}^2 \mathbf{I}_{3 \times 3}
		\end{bmatrix}
	\end{equation}
	
	where:
	\begin{itemize}
		\item $\sigma_a^2$: Accelerometer measurement noise density $\left[\frac{\text{m}^2}{\text{s}^3}\right]$.
		\item $\sigma_g^2$: Gyroscope measurement noise density $\left[\frac{\text{rad}^2}{\text{s}}\right]$.
		\item $\sigma_{ba}^2$: Accelerometer bias random walk noise density $\left[\frac{\text{m}^2}{\text{s}^5}\right]$.
		\item $\sigma_{bg}^2$: Gyroscope bias random walk noise density $\left[\frac{\text{rad}^2}{\text{s}^3}\right]$.
	\end{itemize}
	
	The matrix $\mathbf{L}_c \in \mathbb{R}^{15 \times 12}$ maps these continuous noise sources into the error-state derivative vector $\delta\dot{\mathbf{x}} = [\delta\dot{\mathbf{p}}^T, \, \delta\dot{\mathbf{v}}^T, \, \delta\dot{\boldsymbol{\theta}}^T, \, \delta\dot{\mathbf{b}}_a^T, \, \delta\dot{\mathbf{b}}_g^T]^T$:
	
	\begin{equation}
		\mathbf{L}_c = \begin{bmatrix}
			\mathbf{0}_{3 \times 3} & \mathbf{0}_{3 \times 3} & \mathbf{0}_{3 \times 3} & \mathbf{0}_{3 \times 3} \\
			-\mathbf{R}(\hat{\mathbf{q}}) & \mathbf{0}_{3 \times 3} & \mathbf{0}_{3 \times 3} & \mathbf{0}_{3 \times 3} \\
	...
	The mapping from noise sources to error-state derivatives is defined as follows:
	\begin{itemize}
		\item \textbf{Position Error ($\delta\dot{\mathbf{p}}$):} No direct noise source ($\mathbf{0}_{3 \times 3}$). Position is pure kinematic integration ($\delta\dot{\mathbf{p}} = \delta\mathbf{v}$); uncertainty propagates via system matrix $\mathbf{F}_c$.
		\item \textbf{Velocity Error ($\delta\dot{\mathbf{v}}$):} Accelerometer white noise $\mathbf{n}_a$ maps through the rotated body frame matrix $-\mathbf{R}(\hat{\mathbf{q}})$.
		\item \textbf{Attitude Error ($\delta\dot{\boldsymbol{\theta}}$):} Gyroscope white noise $\mathbf{n}_g$ maps directly into body orientation rates via $-\mathbf{I}_{3 \times 3}$.
		\item \textbf{Accelerometer Bias Error ($\delta\dot{\mathbf{b}}_a$):} Accelerometer bias random walk noise $\mathbf{w}_{ba}$ maps directly via $\mathbf{I}_{3 \times 3}$.
		\item \textbf{Gyroscope Bias Error ($\delta\dot{\mathbf{b}}_g$):} Gyroscope bias random walk noise $\mathbf{w}_{bg}$ maps directly via $\mathbf{I}_{3 \times 3}$.
	\end{itemize}
	
	\subsubsection{First-Order Discretization}
	
	Using a first-order Euler expansion $\delta\mathbf{x}_{k+1} = \delta\mathbf{x}_k + \delta\dot{\mathbf{x}}_k \Delta t$:
	\begin{equation}
		\delta\mathbf{x}_{k+1} = \delta\mathbf{x}_k + \left( \mathbf{F}_c \delta\mathbf{x}_k + \mathbf{L}_c \mathbf{w}_k \right) \Delta t = \left( \mathbf{I} + \mathbf{F}_c \Delta t \right) \delta\mathbf{x}_k + \mathbf{L}_c \mathbf{w}_k \Delta t
	\end{equation}
	
	Extracting the discrete process noise perturbation $\boldsymbol{\eta}_k = \mathbf{L}_c \mathbf{w}_k \Delta t$, we compute $\mathbf{Q}_k = \mathbb{E}[\boldsymbol{\eta}_k \boldsymbol{\eta}_k^T]$ using discrete noise scaling $\mathbb{E}[\mathbf{w}_k \mathbf{w}_k^T] = \frac{\mathbf{Q}_c}{\Delta t}$:
	\begin{equation}
		\mathbf{Q}_k = \mathbb{E}\left[ \left(\mathbf{L}_c \mathbf{w}_k \Delta t\right) \left(\mathbf{L}_c \mathbf{w}_k \Delta t\right)^T \right] = \mathbf{L}_c \left( \frac{\mathbf{Q}_c}{\Delta t} \Delta t^2 \right) \mathbf{L}_c^T = \mathbf{L}_c \mathbf{Q}_c \mathbf{L}_c^T \Delta t
	\end{equation}
	
	Evaluating the product (noting $\mathbf{R}\mathbf{R}^T = \mathbf{I}_3$) yields the full $15 \times 15$ block-diagonal discrete covariance matrix:
	\begin{equation}
		\mathbf{Q}_k = \begin{bmatrix}
			...
			\mathbf{0}_{3 \times 3} & \sigma_a^2 \Delta t \mathbf{I}_{3 \times 3} & \mathbf{0}_{3 \times 3} & \mathbf{0}_{3 \times 3} & \mathbf{0}_{3 \times 3} \\
			\mathbf{0}_{3 \times 3} & \mathbf{0}_{3 \times 3} & \sigma_g^2 \Delta t \mathbf{I}_{3 \times 3} & \mathbf{0}_{3 \times 3} & \mathbf{0}_{3 \times 3} \\
			\mathbf{0}_{3 \times 3} & \mathbf{0}_{3 \times 3} & \mathbf{0}_{3 \times 3} & \sigma_{ba}^2 \Delta t \mathbf{I}_{3 \times 3} & \mathbf{0}_{3 \times 3} \\
			\mathbf{0}_{3 \times 3} & \mathbf{0}_{3 \times 3} & \mathbf{0}_{3 \times 3} & \mathbf{0}_{3 \times 3} & \sigma_{bg}^2 \Delta t \mathbf{I}_{3 \times 3}
		\end{bmatrix}
	\end{equation}
