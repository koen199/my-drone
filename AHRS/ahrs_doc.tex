...
To extract the exponent vector $\phi = \theta \mathbf{u}$ from the $3 \times 3$ rotation matrix $R_{\mathrm{err}}$, the matrix logarithm is computed via the inverse Rodrigues' formula. The rotation angle $\theta$ is derived from the matrix trace:
\[
\theta = \arccos\left(\frac{\text{tr}(R_{\mathrm{err}}) - 1}{2}\right),
\]
where $\theta \in [0, \pi]$. For nominal conditions where $\theta \neq 0$, the rotation vector is isolated using the off-diagonal elements of $R_{\mathrm{err}}$:
\[
\phi = \frac{\theta}{2\sin(\theta)} \begin{bmatrix} R_{32} - R_{23} \\ R_{13} - R_{31} \\ R_{21} - R_{12} \end{bmatrix},
\]
where $R_{ij}$ denotes the element at row $i$ and column $j$. Numerically, when the orientation error approaches zero ($\theta \to 0$), the singularity is bypassed by setting $\phi = [0, 0, 0]^T$ as $\lim_{\theta \to 0} \frac{\theta}{\sin(\theta)} = 1$. 
